...
scheduling_agent = LlmAgent(
    ...
    3. IMMEDIATELY after tools return (in the SAME response), output text:
       - Greet the user
       - Tell them the page count (from tool result)
       - Ask: "How many minutes per day can you dedicate to reading?"
    ...
    - Create Markdown table with columns: Date | Chapter Number | Chapter Name | Page Start | Page End | Number of Pages Read
    - Schedule starts TOMORROW (not today)
    - Estimate 3-5 chapters per section

    For Step 6 (Post to GitHub):
    - Call create_issue tool with:
...
engagement_agent = LlmAgent(
    ...
    3. RESPOND: Your response MUST contain:
       a) The generated content as part of your chat message (for the user).
       b) A tool call to 'create_issue' with the exact same content (for GitHub).
    4. WAIT: Once the create_issue tool returns success, you may finish your response naturally.
    ...
    - REPOSITORY: owner="mosaique258", repo="my_bookish_companion"
